...
public:
    ...
    Intersection getIntersection(Ray ray){
        ...
        // 类似从三角形内的某点作为光线起点, 不应该视t=0为有效的碰撞点
        if (t0 <= 0) t0 = t1;
        ...
        float solveDeviation = 0.01f / ray.direction.norm(); // 这个偏差值和射线方向的模长有关

        //assert((result.coords-center).norm() == radius);
        //assert((result.coords-center).norm() >= radius);
        //assert(fabs((result.coords-center).norm() - radius) < 0.01f); 
        while((result.coords - center).norm() < radius) {
            t0 -= solveDeviation;
            result.coords = Vector3f(ray.origin + ray.direction * t0);
        }
